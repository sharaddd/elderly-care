...
const SOSDialog = ({ open, onOpenChange, isPage = false }: SOSDialogProps) => {
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className={`${isPage ? 'absolute inset-0 w-full h-full max-w-none rounded-none' : 'w-[92%] max-w-[400px] rounded-[32px] fixed'} p-0 overflow-hidden border-none bg-white shadow-2xl z-[100] [&>button:last-child]:hidden`}>
