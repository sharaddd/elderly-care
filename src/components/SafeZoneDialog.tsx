...
const SafeZoneDialog = ({ open, onOpenChange, isPage = false }: SafeZoneDialogProps) => {
    ...
    const content = (
        <div className={`flex flex-col ${isPage ? 'h-full' : 'h-[85vh]'} relative`}>
            {/* Header */}
            <div className="bg-white px-6 pt-6 pb-4 flex items-center justify-between border-b border-gray-100 shrink-0">
                <button
    ...
    if (isPage) {
        return (
            <div className="absolute inset-0 z-[60] bg-slate-50 flex flex-col animate-in slide-in-from-right duration-300 overflow-hidden">
                {content}
            </div>
        );
    }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-[400px] w-[92%] rounded-[32px] fixed inset-0 p-0 overflow-hidden border-none bg-slate-50 shadow-2xl [&>button:last-child]:hidden z-[100]">
                {content}
            </DialogContent>
        </Dialog>
    );
