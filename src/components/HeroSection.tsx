...
const HeroSection = ({ activeMobileView, onViewChange, isMobileOnly = false }: HeroSectionProps) => {
  return (
    <section id="home" className={`relative min-h-[90vh] ${isMobileOnly ? '' : 'md:min-h-screen'} flex items-center pt-0 bg-white ${isMobileOnly ? '' : 'md:bg-transparent'}`}>
      {/* Background image (Hidden on Mobile) */}
      <div className={`absolute inset-0 z-0 ${isMobileOnly ? 'hidden' : 'hidden md:block'}`}>
        ...
        <div className={`absolute inset-0 bg-gradient-to-r from-foreground/90 via-foreground/50 to-transparent z-0 ${isMobileOnly ? 'hidden' : 'hidden md:block'}`} />
      </div>
